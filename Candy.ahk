...
; */
traytip,提示:程序已启动！,选中目标(文字、图片或文件夹)后，快捷键为“Ctrl+右键”或“键盘上的菜单键”
...
Label_Candy_Start:
    ...
    IniRead,candy_DefaultBrowser, %config_iniFile%,Configuration,Default_browser  
    IniRead,程序路径_Autohotkey, %config_iniFile%,Configuration,autohotkey执行文件路径     ;你默认的浏览器，注意不同于系统默认
...
;━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ 根据内容判断或者设定后缀 ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    ...
    ;IfExist,%CandySelected%
     if (InStr(CandySelected,":")>0 and  FileExist(CandySelected)) ;sunwind 1.73
                                                                  ;如果是电脑里面存在的文件
    {
        Flag_isFile:=1
        ...
        IfInString, CandySelected_FileAttrib, D                                                ;Attrib= D ,则是文件夹
        {
            ...
            SplitPath,CandySelected,CandySelected_FileNamenoExt,CandySelected_FilePathOnly,,,CandySelected_FileDriver ;1.73
            StringSplit,Path_array,CandySelected_FilePathOnly,`\     ;sunwind 1.73
            CandySelected_FileFolderName_Uplevel:=Path_array%Path_array0%   ;sunwind 1.73
        }
...
Label_Candy_DrawMenu:
    ...
    IfGreater,CandySelected_StrLen,100         ;菜单第一行，限制字数在20个字
    {
...
Label_Candy_RunCommand:
    ...
    If Flag_isFile=1
    {
      ;if  (SubStr(Candy_Value, 0)!= """" )
        ;  Candy_Value="%Candy_Value%"  ;sunwind解决配置文件里ini命令行兼容双引号      
    ...
    Else If (RegExMatch(Candy_Value,"i)^(Ahk\|)")) ;如果以ahk开头
    {
        IfExist,%Splitted_Candy_Value2%                  ;外部的ahk代码段，你的ahk一般而言可以带参数
            Run %程序路径_Autohotkey% "%Splitted_Candy_Value2%" "%Splitted_Candy_Value3%"
        Else
            Goto Label_Candy_ErrorHandle
    }
...
{
	xxx=
	Flag_defaultBrowser_in_plist=0         ;进程里面是否存在默认浏览器的标志位

	If s_CurrWin_Fullpath Contains %s_UserdBrowsers%                     ;第①步，如果当前激活的窗口是浏览器，则就在本浏览器里面打开
	{
		xxx:=s_CurrWin_Fullpath
	}
	Else Loop,Parse,s_UserdBrowsers,`,                                    ;第②步，看进程里面有没有浏览器
	{
		If A_LoopField=
			Continue
		Process,exist,%A_LoopField%
		If ErrorLevel!=0   ;由于进程隔离的情况，有时候进程里面有exe，也未必能取到，比如在不同desktops的情况下
		{
			WinGet, Running_Browser_Fullpath,ProcessPath,Ahk_PID %ErrorLevel%
			If Running_Browser_Fullpath=
			{
				IfInString,s_DefaultBrowser,%A_LoopField%
				{
					Flag_defaultBrowser_in_plist=1     ;如果取不到进程，同时进程里面又有默认浏览器，就把这个标志置1
				}
				Continue
			}
			Else
			{
				xxx:=Running_Browser_Fullpath
				Break
			}
		}
	}
; 	MsgBox %xxx%  %Flag_defaultBrowser_in_plist%
	If xxx!=       ;如果上面①②两步能取到有效的浏览器，则用这个浏览器
	{
		Run,%xxx%%A_Space%%s_Http%
	}
	Else         ;如果上面①②两步能取不到有效的浏览器
	{
		If Flag_defaultBrowser_in_plist=0     ;如果上面①②两步能取不到有效的浏览器,同时ini定义的系统默认浏览器不在进程里面
		{
			...
			{
				Run,%s_Http%   ;有一种极端情况，假如系统默认浏览器在进程里面，但是运行不起来，就惨了
			}
		}
		Else      ;如果上面①②两步能取不到有效的浏览器,同时ini定义的系统默认浏览器 在 进程里面!
		{
			Run %s_Http% ;有一种极端情况，假如系统默认浏览器在进程里面，但是运行不起来，就惨了
		}
	Return
...
Label_Multifiles:
	...
	Loop,Parse,candyselected,`n,`r
	{
		IfInString, CandySelected_FileAttrib, D                            ;Attrib= D ,则是文件夹
		{
			...
			StringSplit,Path_array,multifiles_filepath,`\     ;sunwind 1.73
           multifiles_FileFolderName_Uplevel:=Path_array%Path_array0%   ;sunwind 1.73
		}
		Else
		{
